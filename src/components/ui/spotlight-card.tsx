  ...
  const getInlineStyles = (): React.CSSProperties & Record<string, string | number> => {
    const baseStyles: React.CSSProperties & Record<string, string | number> = {
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 45) calc(var(--saturation, 80) * 1%) calc(var(--lightness, 60) * 1%) / var(--bg-spot-opacity, 0.08)), transparent
      )`,
  ...
  const beforeAfterStyles = `
    ...
    [data-glow]::before {
      background-image: radial-gradient(
        ...
        hsl(var(--hue, 45) calc(var(--saturation, 80) * 1%) calc(var(--lightness, 50) * 1%) / var(--border-spot-opacity, 1)), transparent 100%
      );
      filter: brightness(2);
    }
    [data-glow]::after {
  ...
  return (
    <>
      ...
      <div
        ...
        className={`${getSizeClasses()} rounded-2xl relative grid shadow-[0_1rem_2rem_-1rem_black] p-6 gap-4 backdrop-blur-[5px] ${className}`}
      >
